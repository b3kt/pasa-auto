...
class TbJasaServiceTest {
    ...
    void findPaginatedWithSearch() {
        ...
        when(query.list()).thenReturn(List.of(new TbJasaEntity()));

        PageResponse<TbJasaEntity> result = service.findPaginated(pr);
    ...
    void testFindAll() {
        ...
        when(repository.listAll()).thenReturn(List.of(jasa));
    ...
    void testFindById_found() {
        TbJasaEntity jasa = new TbJasaEntity();
        when(repository.findByIdOptional(1L)).thenReturn(Optional.of(jasa));
    ...
    void testCreate() {
        TbJasaEntity jasa = new TbJasaEntity();
        doNothing().when(repository).persist(any(TbJasaEntity.class));
        assertNotNull(service.create(jasa));
    }
    ...
    void testUpdate_found() {
        ...
        when(em.merge(any(TbJasaEntity.class))).thenReturn(jasa);
        assertNotNull(service.update(1L, jasa));
    }
    ...
    void testUpdate_notFound() {
        TbJasaEntity jasa = new TbJasaEntity();
        when(repository.findByIdOptional(999L)).thenReturn(Optional.empty());
        assertThrows(jakarta.persistence.EntityNotFoundException.class, () -> service.update(999L, jasa));
    }
