...
const fetchPenjualan = async (paginationData = pagination.value) => {
    ...
    try {
        ...
        // Add status filter - always filter for SPK with SELESAI status
        params.spkStatusFilter = 'SELESAI'
...
const openEditDialog = async (row) => {
    ...
    try {
        ...
        const response = await api.get(`/api/pazaauto/rekap-penjualan/${row.noPenjualan}`)
        if (response.data.success) {
...
const handleSave = async () => {
    ...
    try {
        ...
        let response
        if (isEditMode.value) {
            response = await api.put(`/api/pazaauto/rekap-penjualan/${formData.value.noPenjualan}`, penjualanData)
        } else {
            response = await api.post('/api/pazaauto/rekap-penjualan', penjualanData)
        }
...
const deletePenjualan = async () => {
    deleting.value = true
    try {
        const response = await api.delete(`/api/pazaauto/rekap-penjualan/${itemToDelete.value.noPenjualan}`)
        if (response.data.success) {
            $q.notify({
                type: 'positive',
...
const printPenjualan = async (row) => {
    try {
        const response = await api.get(`/api/pazaauto/rekap-penjualan/${row.noPenjualan}/print`)
        if (response.data.success) {
